if (a_x092969) {
    ...
    if (document.querySelector('.property-page')) {
      ...
      window.addEventListener('scroll', function() {
        ...
        // Inject content into hero section if no search box
        if (document.querySelector('#no-search-content') && document.querySelector('#quicksearch-section.hero-search.search.no-search img')) {
            heroImg = document.querySelector('#quicksearch-section.hero-search.search.no-search img');
            noSearchContent = document.querySelector('#no-search-content');
            insertAfter(noSearchContent, heroImg);
            window.addEventListener("DOMContentLoaded", function () {
                const areaPageHeader = document.querySelector(".area-page-header");
                if (areaPageHeader) {
                    areaPageHeader.classList.remove("hide");
                }
            }, false);
        }
        ...
        function featuredListingShow() {
            ...
            if (document.querySelector('.property.featured')) {
                ...
                // featuredProperties += '   </div><div class="featured-listings-show-slider-arrows"><div class="number-slider"><div class="number-slider-item">01</div><div class="number-slider-item">02</div></div><button class="slick-arrow">âŸ¶</button>';
        ...
        if (document.querySelector(".top-nav")) {
            ...
            var luxnavinner = '<nav id="lux-navbar">' +
                '   <header class="header">';
    
            if (document.querySelector('#content-banner-hide-1')) {
                luxnavinner += '<div id="content-banner-hide-1" class="content-banner--container content-banner" role="banner">';
                luxnavinner += document.querySelector('#content-banner-hide-1').innerHTML;
                luxnavinner += "<style>.content-banner--container {height: 43px;}nav#lux-navbar:not(.showoriginalnav):not(.firsttopnav) {top: -143px;} @media (max-width: 700px){.content-banner--container {height: 75px;} nav#lux-navbar:not(.showoriginalnav):not(.firsttopnav) {top: -150px;}}@media only screen and (max-width: 737px){.hoz-inside {padding: 200px 24px 45px;}}</style></div>";
            }
    
            luxnavinner += '      <div class="container header__container">' +
                '         <div class="navbar">' +
                '            <a href="/">' +
                '              <div class="logo">' +
                '              </div>' +
            ...
            let luxnav = document.createElement("div");
            luxnav.innerHTML = luxnavinner;
    
            document.querySelector('body').prepend(luxnav);
            luxnav.outerHTML = luxnav.innerHTML
    ...
    if (
      document.querySelector("#quicksearch-section") && !document.querySelector("#quicksearch-section.no-search.no-image") && !window.location.pathname.startsWith("/blog/")) {
                    ...
                    var luxNav = document.querySelector("#lux-navbar");
                    ...
                    document.addEventListener('scroll', function () {
                        if (window.scrollY >= 100) {
                            ...
                            luxNav.classList.remove("clearnav");
                            if (scrollYBefore > window.scrollY) {
                                document.querySelector('nav#lux-navbar .header').classList.add("showoriginalnav");
                                ...
                                document.querySelector('nav#lux-navbar .header').classList.remove("showoriginalnav");
                                document.querySelector('nav#lux-navbar').classList.remove("showoriginalnav");
                    ...
                    var topNav = document.querySelector(".header");
                    var luxNav = document.querySelector("#lux-navbar");
                    luxNav.classList.add("noclearnav");
                    if (window.scrollY <= 100) {
                        luxNav.classList.add("firsttopnav");
                    }
                    var scrollYBefore = 0;
                    document.addEventListener('scroll', function () {
                        if (window.scrollY >= 100) {
                            topNav.classList.add("navshadow");
                            luxNav.classList.remove("firsttopnav");
                            if (scrollYBefore > window.scrollY) {
                                document.querySelector('nav#lux-navbar .header').classList.add("showoriginalnav");
                                document.querySelector('nav#lux-navbar').classList.add("showoriginalnav");
                                document.querySelector('.banner-navbar--container').classList.add("showoriginalnav");
                ...
                // fix the banner issue when trigers user modal on mobile
    
                // end of fix the banner issue when trigers user modal on mobile
            }
        }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // Bottom CTA addition
        if (document.querySelector('footer .cta-section') || document.querySelector('.dropsearch')) {
            const footerCtaSection = document.querySelector('footer');
    
            // Create a new div element with the "hero-ctas" class
            const heroCtasDiv = document.createElement('div');
            heroCtasDiv.classList.add('hero-ctas');
    
            // Create a new div element with the "inner-content" class
            const innerContentDiv = document.createElement('div');
            innerContentDiv.classList.add('inner-content');
            innerContentDiv.appendChild(heroCtasDiv);
    
            // Create a new div element with the "luxfullwidth bottomcta" classes and append the innerContentDiv to it
            const wrapperDiv = document.createElement('div');
            wrapperDiv.classList.add('luxfullwidth', 'bottomcta');
            wrapperDiv.appendChild(innerContentDiv);
    
            // Prepend the wrapperDiv inside the footer element
            footerCtaSection.prepend(wrapperDiv);
    
            var bottomNavLinks = '';
            if (document.querySelector('.bottomNav')) {
                var bottomNavLinks = '';
    
                var bottomNavInt = setInterval(function () {
                    if (document.querySelectorAll('.small-12.medium-8.columns.footer-section li') && document.querySelectorAll('.small-12.medium-8.columns.footer-section li').length > 0) {
                        for (var i = 1; i < document.querySelectorAll('.small-12.medium-8.columns.footer-section li').length + 1; i++) {
                            ...
                            var popupclassname = '';
                            if (document.querySelector('.small-12.medium-8.columns.footer-section li:nth-of-type(' + i + ') a').getAttribute('class') && document.querySelector('.small-12.medium-8.columns.footer-section li:nth-of-type(' + i + ') a').getAttribute('class').includes('popup')) {
                                popupclassname = 'popup';
                            }
                            bottomNavLinks += '<a href="' + link + '" class="' + popupclassname + '"><button>' + textContent + '</button></a>';
                        }
                        document.querySelector('.bottomcta .hero-ctas').innerHTML = bottomNavLinks;
                        clearInterval(bottomNavInt);
                    }
                }, 1)
            }
        }
