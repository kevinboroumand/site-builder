if (a_x092969) {
    ...
    // Banner Video for Home ///////////////////////////
    var customURL = "/";
    // BANNER VIDEO
    var useBannerVideo = 1;
    var useVideoHomeOnly = 1;
    var useVideoForMobile = 1;
    var bannerVideoLink = "https://u.realgeeks.media/soldvermont/vanitygeeks/brand/nw-vermont-real-estate.mp4";
    var mobileBannerVideoLink = "https://u.realgeeks.media/soldvermont/vanitygeeks/brand/nw-vermont-real-estate.mp4";
    // BANNER LOGO
    var useCustomBannerLogo = 0;
    var useBannerLogoOnAllPages = 0;
    var useBannerLogoOnMobile = 0;
    var bannerLogoImg = "https://u.realgeeks.media/houston-neighborhoods/logo-new-full-wht-shdw-1.png";
    
    console.log("Is quicksearch-section present?:", $('#quicksearch-section').length);  // Debugging line
        
    if (window.location.pathname === customURL) {
        $('.miranda-wrapper').addClass('home');
    
        if (useBannerVideo === 1) {
            var $bannerVideo = '<div class="video-container" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; overflow: hidden;"><video autoplay loop muted playsinline id="video-background" style="width: 100%; height: 100%; object-fit: cover;"><source src=' + bannerVideoLink + ' type="video/mp4" /></video></div>';
            var $mobileBannerVideo = '<div class="video-container" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; overflow: hidden;"><video autoplay loop muted playsinline id="video-background" style="width: 100%; height: 100%; object-fit: cover;"><source src=' + mobileBannerVideoLink + ' type="video/mp4" /></video></div>';
            
            var x = window.matchMedia("(min-width: 46.01em)");
            
            if (useVideoForMobile === 1) {
                if (x.matches) {
                    $('#quicksearch-section').prepend($bannerVideo);
                    $('.miranda-wrapper').addClass('banner-video mobile-video');
                } else {
                    $('#quicksearch-section').prepend($mobileBannerVideo);
                    $('.miranda-wrapper').addClass('banner-video mobile-video');
                }
            } else {
                $('#quicksearch-section').prepend($bannerVideo);
                $('.miranda-wrapper').addClass('banner-video');
            }
        }
    }
    
    
        // if (!document.querySelector('script[src*="main-footer.js"]')) {
        //     var script = document.createElement('script');
        //     script.onload = function () {
        //         //do stuff with the script
        //     };
        //     script.src = "https://u.realgeeks.media/suburbanilhomes/main-footer.js";
    ...
    <a href="/member/favorite_properties/"><button>My Account</button></a>
        ...
        // add zillow reviews
        if (document.querySelector('.customTestimonials')) {
            if (zillowReviewsEnabled) {
                // grab zillow reviews to testimonials slider
                const url = 'https://www.zillow.com/webservice/ProReviews.htm?zws-id=X1-ZWz176gykcde6j_1u0pu&screenname=' + zillowscreenname + '&count=10&output=json';
                fetch(url)
                    .then((resp) => resp.json())
                    .then(function (data) {
                        let reviews = data.response.results.proReviews.review;
                        return reviews.map(function (review) {
                            let blockquote = document.createElement('div'),
                                description = document.createElement('p');
                            blockquote.classList.add('tile');
                            description.innerHTML = review.description;
                            blockquote.appendChild(description);
                            document.getElementById('zillow-reviews').appendChild(blockquote);
                            if (reviews[reviews.length - 1] == review) {
                                getSlickSlider();
                            }
                        });
                    })
                ...
                if (document.querySelector('.slick-responsive-slider')) {
                    var slickScript = document.createElement('script');
                    slickScript.onload = function () {
                        $('.slick-responsive-slider').slick({
                            dots: true,
    ...
    $(window).on('resize', wrapNavigationButtons);
        ...
        // Switch MLS & Pages elements ///
    
        // $(".pagination_wrapper").insertBefore($(".disclaimer"));
        ...
        $(document).ready(function () {
            ...
            // Check if a .lux-form element exists in the .body-container
            var luxFormExists = bodyContainer.find('.lux-forms').length > 0;
    
            // If a .lux-form element exists, remove it from the footer
            if (luxFormExists) { 
        var luxFormInFooter = footerElement.find('.lux-forms'); 
        var threeCtasContainer = footerElement.find('.three-ctas-container');
        if (luxFormInFooter.length > 0) { 
            luxFormInFooter.remove(); 
        }
        ...
        if ($(".lux-forms").length) {
            $(this).each(function (i, obj) {
                $(document).ready(function () {
                    ...
                    $(this).on('click', '#lux-submit', function (e) {
                        ...
                        setTimeout(function () {
                            ...
                            var luxForms = $(".lux-forms .required");
    
                            if ($(".body-container .lux-forms").length) {
                                luxForms = $(".body-container .lux-forms .required");
                            }
    
                            luxForms.each(function () {
